...
class AlphaTradingStrategy:
    ...
    def plot_cumulative_returns(self, signal_data):
        ...
        factors = '_'.join([x for x in self.factors])

        print("-" * 50 + f"Plot of Portfolio Cumulative Returns vs {self.market_ticker} Cumulative Returns for {factors}" + "-" * 50)
        ...
        portfolio_market_returns.plot(kind='line', figsize=(10, 10),title=f"Backtesting {factors} : Portfolio Cumulative Returns vs {self.market_ticker} Cumulative Returns")
        plt.xlabel('Date')
        plt.ylabel('Cumulative Returns')
        plt.grid(True)
        plt.savefig(os.path.join(FIGURES_DIR, f"Cumulative_Returns_{factors}.png"))
        plt.show()
